...
2. 시드 reset 로그 저장
```
python -m app.db reset 2>&1 | Tee-Object -FilePath docs/verification_seed_reset.txt
```
3. health 응답 저장
```
...
4. 유닛 테스트 로그 저장
```
python -m unittest src/tests/test_seed_smoke.py 2>&1 | Tee-Object -FilePath docs/verification_test_seed_smoke.txt
python -m unittest src/tests/test_enrollment_rules.py 2>&1 | Tee-Object -FilePath docs/verification_test_enrollment_rules.txt
python -m unittest src/tests/test_concurrency_capacity.py 2>&1 | Tee-Object -FilePath docs/verification_test_concurrency_capacity.txt
```
